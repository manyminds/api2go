...
If on the other hand you want to change the response encoding to something other than JSON while taking advantage of the
autogenerated logic you need to provide a `ContentMarshaler` for each `Content-Type` you would like to support.
...
These functions behave exactly like `json.Marshal` and `json.Unmarshal` from the standard `encoding/json` package, but can
work with formats other than JSON. You can pass a map of content marshalers indexed by the MIME content type they're
associated with to the `NewAPIWithMarshalers` function. The generated HTTP handlers will then select the appropriate
format to use based on what the client specified in the `Accept` or `Content-Type` request headers.

Here's an example of providing a custom content marshaler for the [`CBOR`](http://tools.ietf.org/html/rfc7049) format
...
marshalers := map[string]api2go.ContentMarshaler{
  // serve the JSON API via the standard JSON format
  "application/vnd.api+json": api2go.JSONContentMarshaler{},
  // while also allowing clients that speak CBOR
  "application/vnd.api+cbor": &CBORContentMarshaler{handle:&codec.CborHandle{}},
}

api := api2go.NewAPIWithMarshalers("v1", "http://yourdomain.com", marshalers)
```
